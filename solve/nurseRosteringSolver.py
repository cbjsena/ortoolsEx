...
if __name__ == "__main__":
    ...
    if type == 1:
        ...
        result, error_message, processing_time = NurseRosteringSolver(input_data)._set_advanced_solve()
        if result:
            print("✔ 스케줄링 결과:")
        ...
        with open('../test_data/allocation_nurse_data/test2.json', 'r', encoding='utf-8') as f:
            input_data = json.load(f)
        result, error_message, processing_time = NurseRosteringSolver(input_data)._solve()
        if result:
            print("✔ 스케줄링 결과:")
            for day, shift_dict in result["schedule"].items():
                print(f"Day {day + 1}:")
